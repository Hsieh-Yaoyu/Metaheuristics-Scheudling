...
%alpha, beta, rho, Q, turn_w, clear_w, diffusion_rad
%1.68044 3.99775 0.177945 106.823 1.63136 2.67383 1.27466
在此章節中，我們將展示 GA-ACO 演算法在管線佈局問題上的優化效果。
為了驗證 GA 在 ACO 參數調校上的有效性，我們設計了一組對照實驗：一組為「固定參數 ACO」，即使用預設的參數組合
（如 $\alpha=1.0$, $\beta=2.0$, $\rho=0.5$ 等）進行多次獨立執行；另一組為「GA-優化 ACO」，
即讓 GA 自動搜尋最佳參數組合並評估其性能。\\

以下實驗我們使用\texbf{map0}作為測試場景，該地圖包含兩條相互排斥的管線（瓦斯管與電管），
並且每條管線均有一個起點與多個終點的佈局需求。對照組參數設定為 $\alpha=1.0$, $\beta=2.0$, $\rho=0.5$, $Q=100$, $turn\_w=1.0$, $clear\_w=1.0$ 與 $diffusion\_rad=2$。
實驗組使用GA所找尋到的最佳參數組合為 $\alpha=1.68044$, $\beta=3.99775$, $\rho=0.177945$, $Q=106.823$, $turn\_w=1.63136$, $clear\_w=2.67383$ 與 $diffusion\_rad=1.27466$，在迭代中其$Score=167.512$。
